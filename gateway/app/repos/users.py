...
# ------------------------ Users ------------------------
async def create_user(*, email: str, password_hash: str, password_algo: str = "argon2id",
                      display_name: Optional[str] = None, phone: Optional[str] = None) -> dict | None:
    pool = db.get_pool()
...
# --------------------- Auth Sessions -------------------
async def insert_session(*, user_id: str, token_hash: str, expires_at: datetime,
                        ip_address: Optional[str], user_agent: Optional[str]) -> dict:
    """Insert a session row and return minimal data for cookie lifetime, etc."""
    pool = db.get_pool()
    async with pool.connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute(
